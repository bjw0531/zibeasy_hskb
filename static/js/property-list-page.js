...
/**
 * ✅ 터치 바운스 효과 (iOS 고무줄 효과)
 * 맨 위/맨 아래에서 overscroll 시 콘텐츠가 살짝 늘어났다가 돌아옴
 * pull-to-refresh 없이 바운스 시각 효과만 제공
 */
(function initOverscrollBounce() {
    const content = document.querySelector('.list-page-content');
    if (!content) return;

    let startY = 0;           // 터치 시작 Y좌표
    let currentY = 0;         // 현재 터치 Y좌표
    let isBouncing = false;   // 바운스 중 여부
    const MAX_PULL = 80;      // 최대 당김 거리(px)
    const RESISTANCE = 0.35;  // 저항값 (0~1, 낮을수록 뻑뻑)

    // 터치 시작
    content.addEventListener('touchstart', function(e) {
        startY = e.touches[0].clientY;
        // 바운스 중이면 트랜지션 제거 (즉시 반응)
    ...
    content.addEventListener('touchmove', function(e) {
        ...
        // 맨 아래에서 위로 당기기
            ...
            isBouncing = true;
            const pull = Math.max(deltaY * RESISTANCE, -MAX_PULL);
            content.style.transform = 'translateY(' + pull + 'px)';
        }
        // 일반 스크롤 영역이면 바운스 해제
        else if (isBouncing) {
    ...
    content.addEventListener('touchend', resetBounce, { passive: true });
    content.addEventListener('touchcancel', resetBounce, { passive: true });
})();
