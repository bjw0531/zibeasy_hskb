    ...
    return `
        <div class="property-card" data-property-id="${property.idx}" onclick="navigateToDetail('${property.code}')">
            ...
            <div class="property-card-info">
                ...
                ${(timeAgo || distanceBadge) ? `<div class="property-card-time text-xs font-normal" style="${timeStyle}; display: flex; justify-content: flex-end; align-items: center; width: 100%; text-align: right; gap: 8px;"><span>${timeAgo || ''}</span>${distanceBadge ? `<span>${distanceBadge}</span>` : ''}</div>` : ''}
            </div>
        </div>
    `;
}
